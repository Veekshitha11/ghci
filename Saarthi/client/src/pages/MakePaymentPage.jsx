...
const MakePaymentPage = () => {
  ...
  const { walletBalance, setWalletBalance, addTransaction, syncWalletBalance } = useAppState();
  ...
  const refreshBalanceFromServer = async () => {
    try {
      ...
      const latestBalance = response.data?.bankDetails?.balance;
      if (typeof latestBalance === 'number' && !Number.isNaN(latestBalance)) {
        syncWalletBalance(latestBalance);
      }
    } catch (refreshError) {
      console.error('Unable to refresh balance after payment:', refreshError);
    }
  };
