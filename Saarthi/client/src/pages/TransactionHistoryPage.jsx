...
const TransactionHistoryPage = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const apiFilter = timeFilter === 'all' ? 'month' : timeFilter;
      const response = await api.get(`/user/transactions?filter=${apiFilter}`);
  ...
  const applyFilters = (list, time, type) => {
    ...
    return list
      .filter((txn) => {
        ...
        const timeMatch =
          time === 'all' ? true : time === 'week' ? diffDays <= 7 : diffDays <= 31;
  ...
  const hasActiveFilters = timeFilter !== 'all' || typeFilter !== 'all';
